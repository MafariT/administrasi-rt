...
export function generateSuratKeteranganPDF(data: SuratData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Header
    doc.image('public/img/logo-pdf.png', 72, 50, { width: 40 })
    doc.y = 60
    doc
      .font('Helvetica-Bold')
      .fontSize(14)
      .text('RUKUN TETANGGA 12', { align: 'center' })
    doc.fontSize(12).text('PEMERINTAH DESA MENDALO INDAH', { align: 'center' })
    doc.text('KECAMATAN JAMBI LUAR KOTA', { align: 'center' })
    doc.moveDown(1)
    ...
    doc
      .font('Helvetica-Bold')
      .fontSize(12)
      .text(data.letterType.toUpperCase(), { align: 'center', underline: true })
    doc
      .font('Helvetica')
      .fontSize(11)
      .text(`Nomor: ${data.uniqueNumber}`, { align: 'center' })
    doc.moveDown(2)
    ...
    doc
      .font('Helvetica-Bold')
      .text(data.keperluan, { indent: 36, underline: true })
    doc.moveDown(2)
    ...
    doc
      .font('Helvetica-Bold')
      .text('PERIANTO', signatureX, doc.y, { underline: true })
