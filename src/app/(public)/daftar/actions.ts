...
export async function registerWarga(
  values: z.infer<typeof wargaSchema>
): Promise<FormState> {
  ...
  try {
    ...
    const { data: ktpUploadData, error: ktpUploadError } =
      await supabase.storage
        .from('berkas_pendukung')
        .upload(ktpFilePath, ktp_file);
    if (ktpUploadError)
      throw new Error(`Gagal mengunggah KTP: ${ktpUploadError.message}`);
    ...
    if (kkUploadError)
      throw new Error(`Gagal mengunggah KK: ${kkUploadError.message}`);
      ...
      if (insertError.code === '23505') {
        return {
          ...
          errors: [
            {
              path: 'nik',
              message: 'NIK ini sudah ada di sistem.',
            },
          ],
        };
      }
    ...
    return {
      success: true,
      message: 'Pendaftaran berhasil! Data Anda akan segera diverifikasi.',
    };
  } catch (e) {
