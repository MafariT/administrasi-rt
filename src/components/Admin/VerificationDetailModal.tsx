    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="py-4">
          ...
          {!isLoading && !details && (
            <p className="py-8 text-center text-red-500">Gagal memuat data.</p>
          )}
          {details && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
                ...
                <DetailItem
                  label="Tanggal Lahir"
                  value={
                    details.tanggal_lahir
                      ? new Date(details.tanggal_lahir).toLocaleDateString(
                          'id-ID',
                          {
                            year: 'numeric',
                            month: 'long',
                ...
                <DetailItem
                  label="Jenis Kelamin"
                  value={details.jenis_kelamin}
                />
                <DetailItem label="Agama" value={details.agama} />
                <DetailItem
                  label="Status Perkawinan"
                  value={details.status_perkawinan}
                />
                <DetailItem label="Pekerjaan" value={details.pekerjaan} />
                <DetailItem
                  label="Kewarganegaraan"
                  value={details.kewarganegaraan}
                />
                <DetailItem
                  label="Status Tinggal"
                  value={details.status_tempat_tinggal}
                />
                <DetailItem label="No. WhatsApp" value={details.phone_number} />
                <div className="col-span-full">
                  <DetailItem label="Alamat KTP" value={details.alamat_ktp} />
                </div>
                <div className="col-span-full">
                  <DetailItem
                    label="Alamat Domisili"
                    value={details.alamat_domisili}
                  />
                </div>
              </div>
