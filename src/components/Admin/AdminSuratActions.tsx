...
import { approveSuratRequest, rejectSuratRequest } from '@/app/(admin)/admin/surat/actions'
...
export default function AdminSuratActions({ request }: { request: SuratRequest }) {
  ...
  const handleApprove = () => {
    startTransition(() => {
      toast.promise(approveSuratRequest(request.id.toString()), {
  ...
  const rejectSuratAction = (reason: string) => rejectSuratRequest(request.id.toString(), reason);
  ...
  return (
    <>
      ...
      <SuratDetailModal request={request} isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} />
      <RejectionModal
        isOpen={isRejectModalOpen}
        onClose={() => setIsRejectModalOpen(false)}
        action={rejectSuratAction}
      />
    </>
  );
}